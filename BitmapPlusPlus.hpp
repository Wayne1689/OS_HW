...
struct BitmapHeader {
  ...
  std::uint16_t magic;
  /* Magic number for file always BM which is
  0x4D42 */
...
// Sanity check
static_assert(sizeof(BitmapHeader) == 54,
              "Bitmap header size must be 54 bytes");
struct Pixel {
  ...
  explicit constexpr Pixel(const std::int32_t rgb) noexcept
      : r((rgb >> 16) & 0xff), g((rgb >> 8) & 0xff), b((rgb >> 0x0) & 0xff) {}
...
public:
  ...
  Bitmap(const std::int32_t width, const std::int32_t height)
      : m_pixels(static_cast<std::size_t>(width) *
...
public: /* Draw Primitives */
   ...
   */
  void draw_line(std::int32_t x1, std::int32_t y1, std::int32_t x2,
                 std::int32_t y2, const Pixel color) {
   ...
   * Draw a filled rect
   */
  void fill_rect(const std::int32_t x, const std::int32_t y,
                 const std::int32_t width, const std::int32_t height,
                 const Pixel color) {
    if (!in_bounds(x, y) || !in_bounds(x + (width - 1), y + (height - 1)))
      throw Exception("Bitmap::fill_rect(" + std::to_string(x) + ", " +
   ...
   */
  void draw_rect(const std::int32_t x, const std::int32_t y,
                 const std::int32_t width, const std::int32_t height,
                 const Pixel color) {
    if (!in_bounds(x, y) || !in_bounds(x + (width - 1), y + (height - 1)))
      throw Exception("Bitmap::draw_rect(" + std::to_string(x) + ", " +
                      std::to_string(y) + ", " + std::to_string(width) + ", " +
                      std::to_string(height) + "): x,y,w or h out of bounds");
    for (std::int32_t dx = x; dx < x + width; ++dx) {
      m_pixels[IX(dx, y)] = color;
      // top
      m_pixels[IX(dx, y + height - 1)] = color; // bottom
    }
    for (std::int32_t dy = y; dy < y + height; ++dy) {
      m_pixels[IX(x, dy)] = color;
      // left
  ...
  void draw_triangle(const std::int32_t x1, const std::int32_t y1,
    ...
    if (!in_bounds(x1, y1) || !in_bounds(x2, y2) || !in_bounds(x3, y3))
      throw Exception(
          "Bipmap::draw_triangle: One or more points are out of bounds");
  ...
  void fill_triangle(const std::int32_t x1, const std::int32_t y1,
    ...
    if (!in_bounds(x1, y1) || !in_bounds(x2, y2) || !in_bounds(x3, y3))
      throw Exception(
          "Bitmap::fill_triangle: One or more points are out of bounds");
    ...
    const float slope_left =
        static_cast<float>(x_mid - x_top) / (y_mid - y_top);
    const float slope_right =
        static_cast<float>(x_bot - x_top) / (y_bot - y_top);
    // Initialize the starting and ending x-coordinates for each scanline
    std::vector<std::pair<std::int32_t, std::int32_t>> scanlines(y_mid - y_top +
                                                                 1);
    for (std::int32_t y = y_top; y <= y_mid; ++y) {
      const auto x_start =
          static_cast<std::int32_t>(x_top + (y - y_top) * slope_left);
      const auto x_end =
          static_cast<std::int32_t>(x_top + (y - y_top) * slope_right);
    ...
    const float new_slope_right =
        static_cast<float>(x_bot - x_mid) / (y_bot - y_mid);
    // Update the x-coordinates for the scanlines in the lower part of the
    // triangle
    for (std::int32_t y = y_mid + 1; y <= y_bot; ++y) {
      const auto x_start =
          static_cast<std::int32_t>(x_mid + (y - y_mid) * slope_left);
      const auto x_end =
          static_cast<std::int32_t>(x_top + (y - y_top) * new_slope_right);
      scanlines[y - y_top] = {x_start, x_end};
    }
   ...
   */
  void draw_circle(const std::int32_t center_x, const std::int32_t center_y,
                   const std::int32_t radius, const Pixel color) {

    if (!in_bounds(center_x - radius, center_y - radius) ||
        !in_bounds(center_x + radius, center_y + radius))
   ...
   */
  void fill_circle(const std::int32_t center_x, const std::int32_t center_y,
...
public: /* Accessors */
  ...
  * Get pixel at position x,y
  ...
  Pixel &get(const std::int32_t x, const std::int32_t y) {
    if (!in_bounds(x, y))
      throw Exception("Bitmap::get(" + std::to_string(x) + ", " +
                      std::to_string(y) + "): x,y out of bounds");
    return m_pixels[IX(x, y)];
  }
  /**
   * Get const pixel at position x,y
   */
  [[nodiscard]] const Pixel &get(const std::int32_t x,
...
public: /* Operators */
  ...
  bool operator==(const Bitmap &image) const {
    ...
    return (m_width == image.m_width) && (m_height == image.m_height) &&
           (std::memcmp(m_pixels.data(), image.m_pixels.data(),
                        sizeof(Pixel) * m_pixels.size()) == 0);
  }
...
public: /** foreach iterators access */
  [[nodiscard]] std::vector<Pixel>::iterator begin() noexcept {
    return m_pixels.begin();
  }
  [[nodiscard]] std::vector<Pixel>::iterator end() noexcept {
    return m_pixels.end();
  }
  [[nodiscard]] std::vector<Pixel>::const_iterator cbegin() const noexcept {
    return m_pixels.cbegin();
  }
  [[nodiscard]] std::vector<Pixel>::const_iterator cend() const noexcept {
    return m_pixels.cend();
  }
  [[nodiscard]] std::vector<Pixel>::reverse_iterator rbegin() noexcept {
    return m_pixels.rbegin();
  }
  [[nodiscard]] std::vector<Pixel>::reverse_iterator rend() noexcept {
    return m_pixels.rend();
  }
  [[nodiscard]] std::vector<Pixel>::const_reverse_iterator
...
public: /* Modifiers */
  ...
  void set(const std::int32_t x, const std::int32_t y, const Pixel color) {
    if (!in_bounds(x, y)) {
      throw Exception("Bitmap::set(" + std::to_string(x) + ", " +
                      std::to_string(y) + "): x,y out of bounds");
    }
  ...
  Bitmap rotate_90_left() const {
    ...
    for (std::int32_t y = 0; y < m_height; ++y) {
      ...
      for (std::int32_t x = 0; x < m_width; ++x) {
        ...
        finished.m_pixels[(m_width - 1 - x) * m_height + y] =
            m_pixels[y_offset + x];
      }
    }
    return finished;
  ...
  */
  ...
  Bitmap rotate_90_right() const {
    ...
    for (std::int32_t y = 0; y < m_height; ++y) {
      const std::int32_t y_offset = y * m_width; // Precompute row start index
      for (std::int32_t x = 0; x < m_width; ++x) {
        finished.m_pixels[x * m_height + (m_height - 1 - y)] =
            m_pixels[y_offset + x];
      }
    }
    return finished;
  ...
  void save(const std::filesystem::path &filename) const {
    ...
    if (std::ofstream ofs{filename, std::ios::binary}; ofs.good()) {
      ...
      if (!ofs.good()) {
        throw Exception("Bitmap::save(\"" + filename.string() +
                        "\"): Failed to write bitmap header to file.");
      }
      // Write Pixels
      ...
      for (std::int32_t y = m_height - 1; y >= 0; --y) {
        ...
        if (!ofs.good()) {
          throw Exception("Bitmap::save(\"" + filename.string() +
                          "\"): Failed to write bitmap pixels to file.");
        }
      }
    } else
      throw Exception("Bitmap::save(\"" + filename.string() +
  ...
  void load(const std::filesystem::path &filename) {
    ...
    if (std::ifstream ifs{filename, std::ios::binary}; ifs.good()) {
      ...
      if (header->bits_per_pixel != 24) {
        ...
        throw Exception("Bitmap::load(\"" + filename.string() +
                        "\"): Only 24 bitsper pixel bitmaps supported.");
      }
      // Seek the beginning of the pixels data
      // Note: We can't just assume we're there right after we read the
      // BitmapHeader Because some editors like Gimp might put extra information
      // after the header Thanks to @seeliger-ec
      ...
      for (std::int32_t y = m_height - 1; y >= 0; --y) {
        ...
        if (!ifs.good())
          throw Exception("Bitmap::load(\"" + filename.string() +
                          "\"): Failed to read bitmap pixels from file.");
        std::size_t i = 0;
        for (std::int32_t x = 0; x < m_width; ++x) {
          Pixel color{};
          color.b = line[i++];
      ...
      throw Exception("Bitmap::load(\"" + filename.string() +
                      "\"): Failed to load bitmap pixels from file.");
  }
...
private: /* Utils */
   ...
   */
  [[nodiscard]] constexpr std::size_t IX(const std::int32_t x,
                                         const std::int32_t y) const noexcept {
    return static_cast<std::size_t>(x) +
           static_cast<std::size_t>(m_width) * static_cast<std::size_t>(y);
  }
  /**
   * Returns true if x,y coords are within boundaries
   */
  [[nodiscard]] constexpr bool in_bounds(const std::int32_t x,
                                         const std::int32_t y) const noexcept {
